...
function requireParentModule() {
  ...
  const callsites2 = requireCallsites();
  parentModule = (filepath) => {
    const stacks = callsites2();
    if (!filepath) {
...
function requireImportFresh() {
  ...
  const path = require$$0$2;
  const resolveFrom2 = requireResolveFrom();
  const parentModule2 = requireParentModule();
  importFresh = (moduleId) => {
    if (typeof moduleId !== "string") {
      throw new TypeError("Expected a string");
    }
    const parentPath = parentModule2(__filename);
  ...
  (function(exports) {
    ...
    const loadJsSync = function loadJsSync2(filepath) {
      if (importFresh2 === void 0) {
        importFresh2 = /* @__PURE__ */ requireImportFresh();
      }
      return importFresh2(filepath);
    };
...
function requireUtil() {
  ...
  const fs_1 = __importStar(require$$0$1);
  ...
  let ExplorerBase$1 = class ExplorerBase {
    ...
    #validateConfig() {
      ...
      for (const place of config.searchPlaces) {
        ...
        const loader2 = this.config.loaders[extension || "noExt"] ?? this.config.loaders["default"];
        if (loader2 === void 0) {
          throw new Error(`Missing loader for ${getExtensionDescription(place)}.`);
        }
        if (typeof loader2 !== "function") {
          throw new Error(`Loader for ${getExtensionDescription(place)} is not a function: Received ${typeof loader2}.`);
        }
      }
    }
    clearLoadCache() {
...
function requireExplorer() {
  ...
  var __importDefault = Explorer && Explorer.__importDefault || function(mod) {
    return mod && mod.__esModule ? mod : { "default": mod };
  };
  ...
  const util_js_1 = requireUtil();
  let Explorer$1 = class Explorer extends ExplorerBase_js_1.ExplorerBase {
    async load(filepath) {
    ...
    async #loadConfiguration(filepath, contents) {
      ...
      try {
        const loadedContents = await loader2(filepath, contents);
...
function requireExplorerSync() {
  ...
  let ExplorerSync$1 = class ExplorerSync extends ExplorerBase_js_1.ExplorerBase {
    ...
    #loadConfiguration(filepath, contents) {
      ...
      try {
        const loadedContents = loader2(filepath, contents);
        if (path_1.default.basename(filepath, extension) !== "package") {
          return loadedContents;
        }
        return (0, util_js_1.getPropertyByPath)(loadedContents, this.config.packageProp ?? this.config.moduleName) ?? null;
...
function requireDist() {
  ...
  (function(exports) {
    ...
    const Explorer_js_1 = requireExplorer();
    const ExplorerSync_js_1 = requireExplorerSync();
...
function mergeConfigs(defaults2, fileConfig, runtime) {
  let result = { ...defaults2 };
...
function getModuleConfig(moduleName, defaults2) {
...
function getPackageConfig(packageName, defaults2) {
  const fileConfig = loadedConfig || {};
  const runtime = getRuntimeConfig();
  const globalConfig = fileConfig.smrt || {};
