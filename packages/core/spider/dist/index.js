...
async function getScraper(options) {
  ...
  switch (options.scraper) {
    case "basic": {
      const { BasicScraper } = await import("./chunks/basic-BFsLl3kq.js");
...
async function getSpider(options) {
  if (isSimpleOptions(options)) {
    const { SimpleAdapter } = await import("./chunks/simple-4gHLiSQ5.js");
    return new SimpleAdapter(options);
  }
  if (isDomOptions(options)) {
    const { DomAdapter } = await import("./chunks/dom-CITwu_Ug.js");
