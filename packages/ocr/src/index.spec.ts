...
describe('@have/ocr', () => {
  ...
  test('should perform OCR on test PNG image', async () => {
    ...
    if (normalizedResult.length > 0) {
      console.log(
        `Found ${foundWords.length}/${expectedWords.length} expected words:`,
        foundWords,
      );
    ...
    if (
      normalizedResult !==
      expectedText.toLowerCase().replace(/\s+/g, ' ').trim()
    ) {
  ...
  test('should perform OCR on test PNG image with ONNX', async () => {
    ...
    if (normalizedResult.length > 0) {
      console.log(
        `ONNX found ${foundWords.length}/${expectedWords.length} expected words:`,
        foundWords,
      );
      console.log('Normalized result:', normalizedResult);
    }
  ...
  test('should extract actual text with ONNX provider (dedicated ONNX test)', async () => {
    ...
    if (!isAvailable) {
      throw new Error(
        'ONNX OCR provider is not available - this test requires ONNX to be working',
      );
    }
    ...
    console.log(
      'ONNX validation - Detection count:',
      result.metadata?.detectionCount,
    );
