...
class TreeScraper {
    ...
    for (let iteration = 0; iteration < (this.options.maxIterations || 10); iteration++) {
      ...
      for (const selector of selectors) {
        ...
        for (const element of elements) {
          const elementId = await element.evaluate((el, sel) => {
            const getPath = (el2) => {
   ...
   */
  async extractLinksWithTreeExpansionOld(page) {
    const selectors = [
      ...this.DEFAULT_SELECTORS,
      ...this.options.customSelectors || []
    ];
        ...
        for (let iteration = 0; iteration < maxIterations; iteration++) {
          ...
          for (const selector of selectors2) {
            ...
            for (const element of elements) {
              ...
              const isDirectory = element.parentElement?.classList.contains("directory") || element.parentElement?.classList.contains("collapsed");
...
//# sourceMappingURL=tree-coDvq3Pm.js.map
