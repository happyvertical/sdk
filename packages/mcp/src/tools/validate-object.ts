...
function validateImports(code: string, issues: ValidationIssue[]): void {
  const requiredImports = ['SmrtObject'];
  ...
  if (!importLine) {
    issues.push({
      type: 'error',
      message:
        'Missing import from @have/smrt. Add: import { SmrtObject, smrt } from "@have/smrt";',
      code: 'MISSING_IMPORT',
    });
    return;
  }
...
function validateDecorator(
  ...
  if (!decoratorMatch) {
    const hasClass =
      code.includes('class') && code.includes('extends SmrtObject');
    if (hasClass) {
      issues.push({
        type: 'warning',
        message: 'Class extends SmrtObject but missing @smrt decorator',
...
function validateClassStructure(
  ...
  const classMatch = code.match(/class\s+(\w+)\s+extends\s+SmrtObject/);

  if (!classMatch) {
    issues.push({
      type: 'error',
      message: 'No class found extending SmrtObject',
      code: 'NO_BASE_CLASS',
    });
    return;
  }
