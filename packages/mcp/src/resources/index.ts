...
class ResourceManager {
  ...
  private loadResources() {
    ...
    this.addResource({
      uri: 'smrt://docs/core/smrt-object',
      name: 'SmrtObject Documentation',
      description: 'Core SmrtObject class with persistence and AI methods',
      mimeType: 'text/markdown',
    });

    this.addResource({
      uri: 'smrt://docs/core/smrt-collection',
      name: 'SmrtCollection Documentation',
      description: 'Collection management with CRUD operations',
      mimeType: 'text/markdown',
    });

    this.addResource({
      uri: 'smrt://docs/core/smrt-class',
      name: 'SmrtClass Documentation',
      description:
    ...
    switch (type) {
      case 'smrt-object':
        ...
        if (existsSync(filePath)) {
          ...
          content = this.extractDocumentation(sourceCode, 'SmrtObject');
        }
        break;

      case 'smrt-collection':
        filePath = join(SDK_ROOT, 'packages/smrt/src/collection.ts');
        if (existsSync(filePath)) {
          const sourceCode = readFileSync(filePath, 'utf-8');
          content = this.extractDocumentation(sourceCode, 'SmrtCollection');
        }
        break;
      ...
      case 'smrt-class':
        ...
        if (existsSync(filePath)) {
          const sourceCode = readFileSync(filePath, 'utf-8');
          content = this.extractDocumentation(sourceCode, 'SmrtClass');
        }
        break;
  ...
  private async readFieldDocumentation(
    ...
    const filePath = join(SDK_ROOT, 'packages/smrt/src/fields.ts');
  ...
  private extractDocumentation(sourceCode: string, className: string): string {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (line.includes(`class ${className}`) && line.includes('export')) {
